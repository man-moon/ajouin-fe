...
const ignoredFiles = ['src/routes/docs/404.mdx']
 ...
 * 이 부분에서 heading, slug, content 등 추출한다.
 */
...
class MarkdownEmbeddingSource extends BaseEmbeddingSource {
  ...
  constructor(source: string, public filePath: string, public parentFilePath?: string) {
    const path = filePath.replace(/^src\/routes/, '').replace(/\.mdx?$/, '')
    const parentPath = parentFilePath?.replace(/^src\/routes/, '').replace(/\.mdx?$/, '')
